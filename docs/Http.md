...
> Note!!: **DO NOT** use this approach as you have to set everything manually. Using `FileResponse` is the recommended way.

```java
class MyHandler implements Handler {
        @Override
        public HttpResponse handle(HttpRequest request, HttpResponse response) {
            String bodystr = "<h1>Hello</h1>";
            byte[] compressed = Compressor.compress_GZIP(bodystr.getBytes());
            HttpResponse res = new HttpResponseBuilder().scheme("HTTP/1.1")
                    ...
                    .setHeader(Headers.CONTENT_ENCODING.NAME, "gzip");
                    .build();
            return res;
        }
    }
